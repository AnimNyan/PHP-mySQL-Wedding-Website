...
if (!isset($_SESSION['user_id'])) {
    ...
    if ($user_stmt->execute([$_SESSION['user_id']]) && $user_stmt->rowCount() == 1) {
        ...
        // display existing images and allow deletion if nec
        displayImages($product_id, $dbh);
        ?>
...
// display existing images and allow deletion if nec
function displayImages($product_id, $dbh) {
    echo '<pre>', var_dump($_POST) , '</pre>'; // check
    $productImagePath = "C:/xampp/htdocs/product_images/";

    if ($_SERVER['REQUEST_METHOD'] == 'POST' && ($_POST['action']=='Delete the chosen images') 
        && !empty($_POST['delt'])) {
    // delete files in filesystem first
    foreach ($_POST['delt'] as $delt) {
        if (isset($_POST['delt'][$delt]) && !empty($_POST['delt'][$delt])) {
            $query = "select filename from product_images where id=?";
            echo "query is \"$query\"<br>"; // check
            echo $_POST['delt'][$delt]; // check
            $stmt = $dbh->prepare($query);
            if (!$stmt->execute([$_POST['delt'][$delt]])) {
                $err = $stmt->errorInfo();
                echo "Error occurred while selecting filename for product's image# $delt - contact System Administrator. <br>Error is: <b>$err[2]</b>";
                break;
            } else if ($stmt->rowCount() == 1) { // should have only 1 filename
                // *********** NOT WORKING AT THE MOMENT AND HAS BUG FOR THE FOLLOWING ****************
                $filename = $stmt->fetchObject();
                $fullfilename = $productImagePath . $filename;

                echo "Image id# $delt has filename $fullfilename "; // check
 ...
 /*   
    ...
    */    } 
    ...
    // display existing images if they exist, 3 images per row in a table
