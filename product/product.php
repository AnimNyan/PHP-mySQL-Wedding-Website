...
if (!isset($_SESSION['user_id'])) {
    ...
    if ($user_stmt->execute([$_SESSION['user_id']]) && $user_stmt->rowCount() == 1) {
        ...
        // set $category_id if nec
        // This is what is preferred by the user to see what is deleted or updated
        if (isset($_GET['category_id'])) $category_id = $_GET['category_id'];
        if (isset($_POST['category_id'])) $category_id = $_POST['category_id'];

        // if (isset($category_id)) echo '$category_id is ' . $category_id; // check
        ...
        if (isset($category_id) && $category_id != 0) {
            ...
            if ($product_stmt->execute([$category_id])) {
        ...
        if ($product_stmt->rowCount() <= 0) {
            ...
            <form method="post" 
            <?= (isset($_POST['category_id'])) ? 'action=product.php?category_id=' . $category_id . '' : ''; ?>> 
                <input type="submit" name="DelUp" value="Delete or update selected products (Delete has priority over update)" onClick="resetOnce();">
